...
public class ProgressBarSliderUI extends BasicSliderUI {
	...
	public void paintTrack(Graphics g) {
        ...
        if ( slider.getOrientation() == JSlider.HORIZONTAL ) {
            ...
            if(slider instanceof ProgressBarSlider){
            	Font currentFont = slider.getFont();
            	            	
            	FontMetrics fontSizer = SwingUtilities2.getFontMetrics(slider, g,
            			currentFont);
            	      
            	String text = ((ProgressBarSlider)slider).getString();
            	
            	int sWidth = SwingUtilities2.stringWidth(slider, fontSizer, ((ProgressBarSlider)slider).getString());
            	
            	if(sWidth >= trackBounds.height - 3){
            		text = text.substring(0, (int)Math.floor((cw - 2)*text.length()/sWidth) - 3);
            		text += "...";
            		sWidth = SwingUtilities2.stringWidth(slider, fontSizer, text);            		
            	}
            	
            	int sHeight = currentFont.getSize();            	
            	int x = (cw - 2 - sWidth)/2;
            	int y = (trackBounds.height - 2 - sHeight)/2 + sHeight;
            	SwingUtilities2.drawString(slider, g, text, x, y);                            	
            }
            g.translate(-trackBounds.x, -(trackBounds.y + cy));            
        }
        else {
